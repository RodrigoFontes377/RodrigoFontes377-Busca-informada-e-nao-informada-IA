...
function buscaAStar(inicio, objetivo) {
  ...
  caminho.set(inicio, {
    path: [inicio.nome],
    custo: 0,
    heuristica: inicio.heuristica,
  }); // Inicia o caminho com o nome da cidade inicial

  while (filaPrioridade.length > 0) {
    filaPrioridade.sort(
      (a, b) => a.custo + a.heuristica - (b.custo + b.heuristica)
    ); // Ordena a fila de prioridade com base no custo total estimado
    ...
    for (const [vizinho, custoAcao] of no.acoes.entries()) {
      if (!visitados.has(vizinho)) {
        ...
        }); // Adiciona o nome do vizinho ao caminho
        filaPrioridade.push({
          no: vizinho,
          heuristica: vizinho.heuristica,
          custo: novoCusto,
